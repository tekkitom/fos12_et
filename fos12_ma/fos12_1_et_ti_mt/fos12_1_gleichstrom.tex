...
Bei diesem Verfahren werden virtuelle Ströme berechnet. In Teilen der Schaltung, in denen nur ein Kreisstrom fließt ist der Strom des Zweigs identisch zum Kreisstrom der Masche. Wenn mehrere virtuelle Kreisströme durch den selben Zweig fließen, müssen sie entsprechend der Zählrichtung addiert werden. Wenn die Zählrichtung beider Ströme identisch ist, werden sie addiert, sonst subtrahiert. Wenn ein Strom negativ ist, bedeutet dies, dass die Zählrichtung der Masche entgegen der tatsächlichen Stromrichtung ist.

\begin{frame}{Kreisstromverfahren - Baum}
  \begin{columns}
    \begin{column}{.5\textwidth}
      \begin{figure}[htbp]
...
\begin{itemize}
 \item per Hand - z.B. gaußsches Eliminationsverfahren\\ Matrix und SpannungsVektor in Dreiecksform bringen.
...
In Schritt 1.3 (Bild \ref{fig:SchaltungZweiLsgVorschlagTeil1_13}) werden die Widerstände $\text{R}_3$ und $\text{R}_4$ zusammengefasst, da in der Parallelschaltung die selbe Spannung anliegt. $\text{R}_2$ kann nicht mit dem neuen Widerstand $\text{R}_{3||4}$ zusammengefasst werden, da die Spannung an $\text{R}_3$ gesucht wird. Bei einer Zusammenfassung von $\text{R}_2$ und $\text{R}_{3||4}$ kann die Teilspannung nicht mehr direkt bestimmt werden. Ich müsste dann mit Hilfe der Spannungsteiler-Regel die Teilspannung bestimmen.
