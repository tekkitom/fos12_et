...
\date{V 0.1.3 - im Aufbau\\ Stand: \today}%\\
      ...
      \begin{column}{.33\textwidth}
        \begin{figure}[htb]
          \begin{circuitikz}
            ...
            \draw (0,4.5) to (2,4.5);
            \draw (2,4.5) to [short, i>_=$I$] (2,4) to [R=$R_1$,  v=$U_{1}$] (2,2) node[circ]{} to [short, -o] (3,2);
            \draw (2,2) to [R=$R_2$,  v=$U_{2}$] (2,0) node[circ]{} to [short, -o] (3,0);
            \draw (2,0) to (0,0);
          \end{circuitikz}
          %       \caption{Schaltung 1}
  ...
  \begin{frame}{Zwei Spannungsquellen U1 und U2}
    \begin{columns}
      ...
      \begin{circuitikz}
        ...
        \draw (2,2) to [R=$R_2$,  v=$U_2$] (2,0) node[circ]{} node[below]{B}  -- (0,0);
   ...
   \begin{frame}{Schaltung 3}
    ...
    \begin{figure}[htb]
      \begin{circuitikz}
        ...
        \draw (2,1.5)to [R=$R_4$,  v=$U_4$] (2,0) node[circ]{} node[below]{GND}  -- (0,0);
        \draw (2,4) to [R=$R_2$] (4,4);
        \draw (4,4) node[circ]{} node[above]{C}  to [R=$R_5$] (4,2);
        \draw (2,2)  (2,2) node[circ]{} -- (4,2) node[circ]{}  node[right]{D} -- (4,2) to[R=$R_6$]
        (4,0) -- (3,0) -- (2,0);
...
In Schritt 1.2 (Bild \ref{fig:SchaltungZweiLsgVorschlagTeil1_12}) werden die Widerstände $R_5$ und $R_6$ zu einem Widerstand zusammengefasst.
\begin{figure}[htbp]
    \begin{circuitikz}
    \draw (0,0) to [vsource, l=$U_{q1}$] (0,4);
    \draw (0,4) to [R=$R_1$] (2,4) node[circ]{} node[above]{A} to [R=$R_2$] (2,2.5) to [short, i>_=$I_2$]  (2,2) ;
...
In Schritt 1.3 (Bild \ref{fig:SchaltungZweiLsgVorschlagTeil1_13}) werden die Widerstände $R_3$ und $R_4$ zusammengefasst, da in der Parallelschaltung die selbe Spannung anliegt. $R_2$ kann nicht mit dem neuen Widerstand $R_{3||4}$ zusammengefasst werden, da die Spannung an $R_3$ gesucht wird. Bei einer Zusammenfassung von $R_2$ und $R_{3||4}$ kann die Teilspannung nicht mehr direkt bestimmt werden. Ich müsste dann mit Hilfe der Spannungsteiler-Regel die Teilspannung bestimmen.
...
In Schritt 4 (Bild \ref{fig:SchaltungZweiLsgVorschlagTeil1_14}) kann ich die neuen Widerstände berechnen und die Teilspannung an $R_3 \rightarrow U_3'$.

Zur Berechnung der Spannung an $R_1$ benötige ich als Berechnungs-Größe den Gesamtwiderstand von $(R_2+R_{3||4})||R_{5+6}$.
\begin{align}
  R_{5+6} &= R_5 + R_6 = 560 \Omega + 180 \Omega = 740 \Omega\\
  R_{3||4} &=  \frac{1}{\frac{1}{R_3}+\frac{1}{R_4}} = \frac{1}{\frac{1}{270 \Omega}+\frac{1}{470 \Omega}} = 171,49 \Omega\\
  R_{2+3||4} &= 220\Omega + 171,49\Omega\\
  &= 391,49\Omega
\end{align}
Berechnung des Gesamtwiderstands $R_2+R_{3||4}||R_{5+6}$ als Hilfsgröße:
\begin{align}
  (R_2+R_{3||4})||R_{5+6} &= \frac{1}{\frac{1}{R_2 + R_{3||4}}+\frac{1}{R_{5+6}}}\\
   &= \frac{1}{1}{220 \Omega + 171,49 \Omega}+\frac{1}{740 \Omega} = 256,04 \Omega
\end{align}

Anwendung der Spannungsteiler-Regel für $R_1$ zu $R_2+R_{3||4}||R_{5+6}$. Die Schaltung zwischen den Knoten A und B nenne ich in den folgenden Formeln $R_{AB}$.

\begin{align}
  \frac{U_{AB}'}{U_{q1}}  &= \frac{R_{AB}}{R_1 + R_{AB}}\\
  U_{AB}' &= U_{q1} * \frac{R_2+R_{3||4}||R_{5+6}}{R_1+R_2+R_{3||4}||R_{5+6}}\\
  U_{AB}' &= 12\,V * \frac{256,04 \Omega}{256,04 \Omega + 100 \Omega}\\
  U_{AB}' &= 8,63\,V
\end{align}

\begin{align}
  \frac{U_3'}{U_{AB}'} &= \frac{R_{3||4}}{R_2+R_{3||4}}\\
  U_3' &= U_{AB}'* \frac{R_{3||4}}{R_2+R_{3||4}}\\
  U_3' &= 8,63\,V *\frac{171,49 \Omega}{220 \Omega+ 171,49 \Omega}\\
  &= 3,78\,V
\end{align}
...
\subsubsection{Schaltung 2 - nur Quelle 2 aktiv}
  \begin{frame}<presentation>{Schaltung 2- nur Quelle 2 aktiv}
    \begin{columns}
      \begin{column}{.6\textwidth}
        \begin{figure}[htbp]
          \begin{circuitikz}
...
\end{figure}
Für die Berechnung von $U_{3||4}''$ berechne ich wieder zuerst den Widerstand $R_{AB}''$ aus $R_1||(R_2+R_{3||4}) = 79,65 \Omega$.

Anschließend berechne ich unter zweimaliger Anwendung der Spannungsteiler-Regel die Spannung $U_3''$.

\begin{align}
  \frac{U_{AB}''}{U{q2}} &=\frac{R_{AB}}{R_{AB} + R_{5+6}}\\
  U_{AB}'' &= U_{q2}*\frac{R_{AB}}{R_{AB} + R_{5+6}}\\
  U_{AB}'' &= 15\,V*\frac{79,65 \Omega}{79,65 \Omega + 740 \Omega}\\
  &= 1,46\,V
\end{align}

\begin{align}
  \frac{U_{3||4}''}{U{AB}} &=\frac{R_{3||4}}{R_{3||4} + R_{2}}\\
  U_{3||4}'' &= U_{AB}*\frac{R_{3||4}}{R_{3||4} + R_{2}}\\
  U_{3||4}'' &= 1,46\,V*\frac{171,49 \Omega}{171,49 \Omega + 220 \Omega}\\
  &= 0,64\,V
\end{align}
Berechnung von $U_{3}$
...
\subsubsection{Schaltung 3 - nur Quelle 1 aktiv}
  ...
  \begin{frame}<presentation>{Schaltung 3 - nur Quelle 1 aktiv}
    \begin{columns}
      \begin{column}{.6\textwidth}
        \begin{figure}
          \begin{circuitikz}
...
%             \only<4->{\draw (2,1.5) -- (2,2);}% node[circ]{} -- (4,2);}
    ...
    \draw (6,0) to [short, -*] (4,0);
  \end{circuitikz}
  \caption{Überlagerung, Schaltung 3, (Schritt 1.1)}
  \label{fig:SchaltungDreiLsgVorschlagTeil11}
\end{figure}

In Schritt 1.1 (Bild \ref{fig:SchaltungDreiLsgVorschlagTeil11}{fig:SchaltungDreiLsgVorschlagTeil11}) wurde Quelle 2 durch einen Kurzschluss ersetzt.
...
\begin{figure}[htbp]
  \begin{circuitikz}
    ...
    \draw (4,4) node[circ]{} node[above]{C} to [R=$R_5$] (4,2);
    \draw (2,2) node[circ]{} -- (4,2);
    \draw (4,2) node[circ]{} node[right]{D} -- (4,2) to[R=$R_6$] (4,0);
    \draw (4,0) -- (3,0) -- (2,0);
    \draw (4,4) -- (6,4);
    \draw (6,4) to [R=$R_{3+7}$] (6,2);
    \draw (6,2) -- (6,0);
    \draw (6,0) to [short, -*] (4,0);
  \end{circuitikz}
  \caption{Überlagerung, Schaltung 3, (Schritt 1.2)}
  \label{fig:SchaltungDreiLsgVorschlagTeil12}
\end{figure}

In Schritt 1.2 (Bild \ref{fig:SchaltungDreiLsgVorschlagTeil12}) wurden die Widerstände 3 und 7 zusammengefasst ($R_{3+7}$)
...
\begin{figure}[htbp]
  \begin{circuitikz}
    ...
    \draw (6,0) -- (4,0);
  \end{circuitikz}
  \caption{Überlagerung, Schaltung 3, (Schritt 1.3)}
  \label{fig:SchaltungDreiLsgVorschlagTeil13}
\end{figure}

In Schritt 1.3 (Bild \ref{fig:SchaltungDreiLsgVorschlagTeil13}) wurden die Widerstände 4 und 6 als Parallelschaltung zusammengefasst.
...
\begin{figure}[htbp]
  \begin{circuitikz}
    ...
    \draw (6,0) -- (4,0);
  \end{circuitikz}
  \caption{Überlagerung, Schaltung 3, (Schritt 1.4)}
  \label{fig:SchaltungDreiLsgVorschlagTeil14}
\end{figure}

In Schritt 1.4 (Bild \ref{fig:SchaltungDreiLsgVorschlagTeil14}) wurde der Widerstand R5 in einer anderen Position neu eingezeichnet, um die Parallelschaltung zu R2 zu verdeutlichen.
...
Im finalen Schritt 1.5 wurden die Parallelschaltung aus R2 und R5 mit der Reihenschaltung R3 und R7 zusammengefasst. Jetzt kann die Spanung an $R_4||R_6 ||R_{2||5)+3+7}$ berechnet werden.

  \begin{frame}<presentation>{Schaltung 3 - nur Quelle 2 aktiv}
    \begin{columns}
      \begin{column}{.6\textwidth}
        \begin{figure}
          \begin{circuitikz}
...
\begin{align}
  R_3 + R_7 &= 270\Omega + 120 \Omega = 390 \Omega\\
  R_2 || R5 &= \frac{1}{\frac{1}{220 \Omega} + \frac{1}{470 \Omega}} = 149,86 \Omega\\
  R_{(2||5)+3+7} &= 390 \Omega +149,86 \Omega = 539,86 \Omega\\
  R_4 || R_6 &= \frac{1}{\frac{1}{470 \Omega} +\frac{1}{560 \Omega}} = 255,53 \Omega\\
  R_{(2||5)+3+7}||R_{4||6} &= \frac{1}{\frac{1}{539,86}+\frac{1}{255,53 \Omega}} = 173,44 \Omega
\end{align}

Nach der Formel Spannungsteiler (Teilspannung / Gesamtspannung = Teilwiderstand / Gesamtwiderstand) ergibt sich:
\begin{align}
\frac{U_4'}{U_{q1}} &= \frac{R_{(2||5)+3+7}||R_{4||6}}{R_1+R_{(2||5)+3+7}||R_{4||6}}\\
U_4' &= U_{q1} * \frac{R_{(2||5)+3+7}||R_{4||6}}{R_1+R_{(2||5)+3+7}||R_{4||6}}\\
U_4' &= 12\,V * \frac{173,44 \Omega}{100\Omega + 173,44 \Omega} \\
U_4' &= 7,61V
\end{align}

Für Quelle 2 ergibt sich damit:
...
\begin{figure}[htbp]
  \begin{circuitikz}
    ...
    \draw (0,4) to [R=$R_1$] (2,4) node[circ]{} node[above]{A} to (2,2);
    \draw (2,2)to [R=$R_{4||6}$,  v=$U_4$] (2,0) node[circ]{} node[below]{GND}  -- (0,0);
    \draw (2,4) -- (6,4) to [R=$R_{(2||5)+3+7}$] (6,2);
    \draw (4,0) -- (3,0) -- (2,0);
    \draw (6,2) to [vsource, l=$U_{q2}$] (6,0);
    \draw (6,0) -- (4,0);
  \end{circuitikz}
...
\begin{align}
  ...
  R_{(2||5)+3+7} &= 390 \Omega +149,86 \Omega = 539,86 \Omega\\
  R_4 || R_6 &= \frac{1}{\frac{1}{470 \Omega} +\frac{1}{560 \Omega}} = 255,53 \Omega\\
  R_{1}||R_{4||6} &= \frac{1}{\frac{1}{100 \Omega}+\frac{1}{255,53 \Omega}} = 71,87 \Omega\\
  \frac{U_4''}{U_{q2}} &= \frac{R_{1}||R_{4||6}}{R_{(2||5)+3+7}+R_1||R_{4||6}}\\
  U_4'' &= U_{q2} * \frac{R_{1}||R_{4||6}}{R_{(2||5)+3+7}+R_1||R_{4||6}}\\
  U_4'' &= 15\,V * \frac{71,87 \Omega}{255,53 \Omega + 71,87 \Omega}\\
  U_4'' &= 1,76\,V\\
  U_4 &= U_4' + U_4''\\
  U_4 &= 7,61\,V + 1,76\,V\\
  U_4 &= 9,37\,V
