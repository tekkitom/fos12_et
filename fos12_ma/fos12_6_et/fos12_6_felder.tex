...
\date{V 0.3.0 - im Aufbau\\ Stand: \today}%\\
...
\begin{frame}{Übersicht}
  \begin{description}
   ...
   \item[Magnetisches Feld] \hyperlink{Teil_2MagnetischesFeld}{\beamergotobutton{Teil magnetisches Feld}}
   \item[Lösungsvorschläge] \hyperlink{LoesungsVorschlaege}{\beamergotobutton{Lösungsvorschläge zu einigen Aufgaben}}
  \end{description}
  \hypertarget{Uebersicht}{}
\end{frame}

\part{Elektrisches Feld}
...
\subsection[Energieerhaltung]{Energieerhaltung und Einheit}
% \only<presentation>{
%   \begin{frame}
%     \frametitle{Inhalt}
%
...
\subsection[Ladungen]{Abmaße von Ladungen}
% \only<presentation>{
%   \begin{frame}
%     \frametitle{Inhalt}
%
...
\subsection[Schalt. von Cs]{Schaltung von Kondensatoren}
...
Wenn man mehrere Kondensatoren in Reihe schaltet, verteilen sich die Ladungen auf den Platten gleichmäßig (siehe Bild \ref{fig:2CsInReihe})\footnote{Die Aufteilung erfolgt in der Realität nicht absolut gleichmäßig. "`Randeffekte"' werden in diesen Arbeitsmaterialien ausgeblendet}.
...
\subsection{Aufgaben zu Schaltung von Kondensatoren}
\begin{frame}
    \begin{figure}[htb]
    Berechnen Sie die Gesamtkapazität der Schaltung mit den Werten aus a und b.\\
      \begin{circuitikz}
        %\draw (0,0) to[vsource, l=$U_{q1}$] (0,2);
        \draw (0,2) node[ocirc]{} to[C=$C_1$] (4,2) node[circ]{} -- (4,1.5) to[C=$C_3$] (4,0) node[circ]{};
        \draw (4,1.5) node[circ]{} -- (2.5,1.5) to[C=$C_2$] (2.5,0) node[circ]{};
%        to[C=$C_2$] (2,0) node[circ]{}  -- (0,0);
        %\draw (2,2) -- (4,2) node[circ]{} to[C=$C_3$]
        %(4,0) node[circ]{}  --  (2,0);
        \draw (4,2) to[C=$C_4$] (6,2);
        \draw (6,2) to[C=$C_5$] (6,0) --
        %(6,2) to[vsource, l=$U_2$] (6,0) --
        (6,0) -- (4,0) -- (2.5,0) -- (0,0)node[ocirc]{};
      \end{circuitikz}
      %       \caption{Schaltung 1}
      \label{fig:AufgabeSchaltung Cs1}
    \end{figure}
    \begin{description}
        \item[a)] $\text{C}_1 = 220 \mu F, \text{C}_2 = 330 \mu F, \text{C}_3 = 470\mu F, \text{C}_4 = 22 \mu F, \text{C}_5 = 47 \mu F$
        \item[b)] $\text{C}_1 = 220 nF, \text{C}_2 = 330 nF,  \text{C}_3 = 470nF, \text{C}_4 = 22 \mu F, \text{C}_5 = 47 \mu F$
\end{description}
    Das Ergebnis soll jeweils in $\mu F$ und $nF$ angegeben werden. Lösen Sie die Schaltung von innen nach außen schrittweise auf. berechnen Sie die Ersatz-Werte für die Zwischenschritte und runden Sie auf 2 Nachkommastellen. Verwenden Sie später die berechneten Werte in der nächsten Stufe der Schaltung.
\end{frame}

\section{Elektrisches Feld}
\subsection{Ladung, E-Feld}
...
\begin{frame}{Ladungen, elektrisches Feld}
 Recherchieren Sie die folgenden Begriffe und notieren sich eine eigene Erklärung (Umfang je ca. 2 - 4 Sätze).
  \begin{itemize}
  \item Elektrisches Feld.
  \item Elektrische Ladung
  \item elektrische Feldstärke
  \item Elektrisches Feld zwischen zwei parallelen Kondensatorplatten.
 \end{itemize}
Hinweis: Die Verwendung von LLM (ChatGPT, \dots) ist zur Sammlung von Stichworten gut geeignet. LLMs halluzinieren teilweise. Eine Recherche bei vertrauenswürdigen Seiten ist besser.
\end{frame}

\subsection{E-Feld, Spannung}
\begin{frame}{Definition und Formel}
\begin{itemize}
  \item Beginn bei pos. Ladung 
  \item Ende bei neg. Ladung
  \item $\vec{E} = \frac{1}{4\pi \varepsilon_0} \cdot \frac{Q}{r^2}\cdot\vec{e}$\\ r = Abstand,  $\varepsilon_0 = 8,85 \cdot 10^{-12}\frac{As}{Vm}$, Q = Quellladung, \\$\vec{e}$ = Vektor der Länge 1, für Richtungsangabe. \\Elektrisches Feld $\vec{E}\left[\frac{V}{m}\right]$
\end{itemize} 
\end{frame}

\begin{frame}{Aufgaben zu E-Feld}
\begin{tabular}{c|c|c}
    Ladung Q& Abstand r& Feldstärke $\vec{E}$\\ \hline 
    $150\ mC$ & 20\,m, 50\,m, 1000\ m & \\ \hline
    $3,2\cdot 10^{-6}\ C$ & 2\,m, 5\,m, 10\ m & \\ \hline  
    $4,7\ nC$ & & 54 V/m, 161,9 V/m, 202,3 V/m\\ \hline
    & 6 m& 124,1 V/m
\end{tabular}
\end{frame}

\begin{frame}{Ergebnisse zu Aufgaben E-Feld}
\begin{tabular}{c|c|c}
    Ladung Q& Abstand r& Feldstärke $\vec{E}$\\ \hline 
    $150\ mC$ & 20\,m, 50\,m, 1000\ m & 13,48\ MV, 2,157\ MV, 5,392\ kV\\ \hline
    $3,2\cdot 10^{-6}\ C$ & 2\,m, 5\,m, 10\ m & 28,76\ kV, 4,602\ kV, 1,150\ kV\\ \hline  
    $4,7\ nC$ & 1,769\ m, 1,022\ m, 0,913\ m& 54 V/m, 161,9 V/m, 202,3 V/m\\ \hline
    496,6\ nC& 6 m& 124,1 V/m
\end{tabular}
\end{frame}
\only<presentation>{
...
\column{.7\textwidth}
 ...
 Gegeben seien drei Ladungen $Q_1, Q_2\ \text{und} \ Q_3$. Alle Ladungen sind ideal punktförmig und haben den Wert $10\mu C$.
  ...
  Die Ladungen befinden sich an folgenden Punkten: $Q_1= \left(\begin{array}{c} 2 \\
2 \end{array}\right)$,
  $Q_2= \left(\begin{array}{c} 6 \\ 2 \end{array}\right)$,
  $Q_3= \left(\begin{array}{c} 4 \\ 4 \end{array}\right)$. Das Koordinatensystem ist im m skaliert (eine Einheit = 1\,m). $\varepsilon = \varepsilon_0 = 8,85 -10^{-12}\frac{As}{Vm}$
  Berechnen Sie die resultierende Kraft auf $Q_3$, die von $Q_1 \ \text{und}\ Q_2$ ausgeht.
\end{columns}
\only<2>{$\text{Q}_3 = \text{Q}_{13}+\text{Q}_{23}$\\
$\text{Q}_{13} = 0,225\ N, \text{Q}_{23} = 224,69\cdot 10^{-3} N, \text{Q}_3 =449,38 \ mN$ Wirkung in X-Richtung}
\end{frame}

\begin{frame}{Kraft auf eine Ladung}
\framesubtitle{Lösungsvorschlag - Teil 1}
Gegeben: $Q1 = Q2 = Q3 = 10 \cdot 10^{-6}C$ \\
\begin{align}
    \text{Distanz: }Q_3-Q_1  &= \left(\begin{array}{c} 4 \\ 4 \end{array}\right)- \left(\begin{array}{c} 2 \\ 2 \end{array}\right)
= \left(\begin{array}{c} 4-2 \\ 4-2 \end{array}\right)= \left(\begin{array}{c} 2 \\ 2 \end{array}\right)
\end{align}

\begin{align}
   \text{Abstand/Betrag: } |Q_3-Q_1| =\sqrt{2^2+2^2}=2,83=2\sqrt{2}
\end{align}
%\\
\begin{align}
 \text{Kraft: } \vec{F} &= \frac{Q_1 \cdot Q_3}{4 \cdot \pi \cdot \varepsilon\cdot r^2}
 \\
 \vec{F} &= \frac{(10\cdot 10^{-6})^2}{4 \cdot \pi \cdot \varepsilon_0\cdot (2\sqrt{2})^2}=0,112\ N
 \end{align}
\end{frame}

\begin{frame}{Kraft auf eine Ladung}
\label{KraftAufLadungAufgabe1-3}
 \framesubtitle{Aufgaben}
\begin{columns}
    \column{.5\textwidth}
 \begin{figure}
  \begin{tikzpicture}
      \draw[step=.5 cm, gray!90, very thin] (0,0) grid (4,3);
      \draw[->] (-.5,0) -- (5,0);
      \draw[->] (0,-.5) -- (0,4);
      %\draw node[circ] at (0,0){};
      \filldraw[white] (1,1) circle [radius=5pt]
...
%\begin{align}
 ...
 Distanz x-Richtung %}
 \\
 $|Q_3-Q_1| =\sqrt{2^2+0^2}=2$\\
 %\text{
 Winkel\\
 $tan(\alpha) =\frac{\Delta y}{\Delta x}=\frac{2}{2}\rightarrow 45^\circ$\\
 Kraft (x-Richtung von Ladung 1 auf Ladung 3)%} 
 \\
 $\vec{F}_{x13} = \frac{Q_1 \cdot Q_3}{4 \cdot \pi \cdot \varepsilon\cdot r^2}$
 \\
 $\vec{F}_{x13} = \frac{(10\cdot 10^{-6})^2}{4 \cdot \pi \cdot \varepsilon_0\cdot 2^2}=0,225\ N$\\
 Weitere Kräfte\\
 $\vec{F}_{y13} = \vec{F}_{x13}\ (\text{da}\ \alpha_{Q1,Q3}=45^\circ)$\\
 $\vec{F}_{x23} =\vec{F}_{x13}$
\\
$\vec{F}_{y13} = -\vec{F}_{y23}$
%\end{align}
\end{columns}
\end{frame}

\begin{frame}{Kraft auf Ladung}
\framesubtitle{Lösungsvorschlag - Teil 2}
Kraft nur in x-Richtung relevant\footnote{in y-Richtung heben sich $F_{Q_1-Q_3}$ und $F_{Q_2-Q_3}$ auf.}

\begin{align}
 \text{Distanz } |Q_3-Q_1| &=\sqrt{2^2+0^2}=2\\
 \text{Kraft } \vec{F} &= \frac{Q_1 \cdot Q_3}{4 \cdot \pi \cdot \varepsilon\cdot r^2}
 \\
 \vec{F} &= \frac{(10\cdot 10^{-6})^2}{4 \cdot \pi \cdot \varepsilon_0\cdot 2^2}=0,225\ N
\end{align}
\end{frame}
\subsection{Vektoren}
\only<presentation>{
...
\subsection[Überlagerung E]{Überlagerung von elektrischen Feldern}
% \only<presentation>{
%   \begin{frame}
%     \frametitle{Inhalt}
%
...
% \clearpage
\part{Magnetisches Feld}
\begin{frame}
  \partpage
  \hypertarget{Teil_2MagnetischesFeld}{}
  \hyperlink{Uebersicht}{\beamerreturnbutton{Zur Übersicht}}
\end{frame}
...
\begin{frame}<beamer>
  \frametitle{Inhalt}
    \tableofcontents
     Folgt noch:\\
    \begin{itemize}
     \item Induktion (P) (Magnetischer Fluss (Phi) (P), Flussdichte (B) (P))
    \item Spule (P) (Ein- und Ausschaltvorgang) (P)
    \end{itemize}
    P = Prüfungsrelevant (Obligatorische Inhalte für die Prüfung)
\end{frame}

% \begin{frame}{Überblick Magnetisches Feld}
%   \begin{description}
%     \item[Magnetisches Feld] allgemein, magnetische Pole, Unterschiede zum elektrischen Feld
%     \item[Feldlinienbilder] Feldlinien
%     \item[Formen von Magneten]
%     \item[Magnetische Kraftwirkung]
%     LORENTZ-Kraft
%     \item[Selbstinduktion] Magnetischer Fluss (Phi), Flussdichte (B)
%     \item[Spule] Ein- und Ausschaltvorgang
%   \end{description}
% \end{frame}
...
\begin{frame}{Beispiele Magnetfeld}
 \begin{itemize}
  ...
  \item Funk (Elektro- magnetisches Feld)
...
\begin{frame}{magnetische Pole}
 \begin{itemize}
  ...
  \item Nordpol (Austritt), Südpol (Eintritt der Feldlinien)
  \item Bei Dauermagnet, elektrischen Magnet (Spule, Leitung, \dots)
 \end{itemize}
\end{frame}
...
\begin{frame}{Unterschiede zum elektrischen Feld}
  \begin{description}
   ...
   \item[Magnetisches Feld] von Nordpol zu Südpol (außerhalb des Bauteils)
   \item[bei Elektrischem Strom] Magnetfeld steht senkrecht auf elektrischem Feld\footnote{E-Feld ist parallel zu Längsachse der Leitung, Magnetfeld ist kreisförmig (UZ) drehend um Leitung / Spule}.
  \end{description}
\end{frame}

\section{Feldlinienbilder}
...
\begin{frame}{Spule und Magnetfeld - Durchflutung}
  \begin{itemize}
   \item Magnetfeld ist vom Strom (I) abhängig.
   \item Magnetfeld ist von Windungsanzahl (N) abhängig.
   \item $\theta = I\cdot N$
   \item wenige Windungen und großer Strom $\hat =$ viele Windungen und wenig Strom.
  \end{itemize}
\end{frame}


\begin{frame}{Überlagerung von Magnetfeldern / Beeinflussung}
  \begin{columns}
   ...
   \end{column}
    ...
    \begin{itemize}
     \item Senkrechte Linie = Leiterbahn (keine Spule)
     \item Gitter ist GND (Masse)
     \item Magnetfelder überlagern sich (addieren bei gleicher Richtung / subtrahieren bei unterschiedlicher Richtung)
...
\section[Formen]{Formen von Magneten}
\begin{frame}{Formen von Magneten}
  \begin{itemize}
  \item Permanentmagnet (z.B. Eisen)
  \item Elektromagnet (Spule, Strom durchflossen)
  \end{itemize}
\end{frame}

\section[Mag. Kraft]{Magnetische Kraftwirkung (LORENTZ-Kraft, Selbstinduktion)}
\begin{frame}{Magnetische Kraftwirkung}
\begin{itemize}
 \item Kraftwirkung ähnlich E-Feld
 \item Wirkung auf magnetische Metalle
 \item Wirkung auf Elektromagnete
 \item Bei Leitung\footnote{Stromdurchflossene Leitung}: senkrecht zu Leitung
 \item gleiche Richtung = Anziehung
\end{itemize}
\end{frame}

\begin{frame}{LORENTZ-Kraft}
\begin{itemize}
 \item Wirkung von stromdurchflossener Leitung
 \item Wirkung auf andere Leitungen (Stromdurchflossen)
 \item Wirkung auf andere Magnete
\end{itemize}
Berechnung erfolgt mit Kreuzprodukt der Vektoren.
\end{frame}

\begin{frame}{Info Kreuzprodukt (Vektoren)}
 Berechnung: Ein Vektor, der senkrecht auf der Ebene aus zwei anderen Vektoren steht.
 \begin{align}
 a\times b &= c \\
 \left(
 \begin{array}{c}
...
\begin{frame}{Magnetisches Feld}
  \begin{columns}
    \begin{column}{.5\textwidth}
      \begin{itemize}
        \item Magnetische Feldstärke (auch Erregung)
        \item Formelzeichen H
        \item $H=\frac{I}{2\pi \cdot r}$
        \item Einheit $\frac{A}{m}$
      \end{itemize}
    \end{column}
    \begin{column}{.5\textwidth}
      \includegraphics[scale=.5]{magFeld_h_2.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Magnetischer Fluss (Phi, $\Phi$)}
  \begin{columns}
    \begin{column}{.5\textwidth}
      \begin{itemize}
        \item Beschreibung magnetisches Feld (im Raum)
        \item Qualitative Angabe zu Feldlinien (nicht quantitativ)
        \item analog zu elektrischem Strom
        \item Magnetischer Widerstand (Abhängig von
        \begin{itemize}
        \item $\mu = \mu_0 + \mu_r$ Magnetische Feldkonstante oder Permeabilitätskonstante
        \item A (Fläche)
        \end{itemize}
        $\mu_0=\mu_0 = 1,2566 \cdot 10^{-6} \frac{N}{A^2}$
        \item magnetische Spannung $V_m = R_m \cdot \Phi$
      \end{itemize}
    \end{column}
    \begin{column}{.5\textwidth}
      \includegraphics[scale=.5]{magFeld_b_2.png}
    \end{column}
  \end{columns}
\end{frame}

$\mu$ und $\mu_0$ sind nicht gleich dem Formelzeichen für Mikro ($\mu$ als Vorsatz zu einer Einheit).

% \subsection{Magnetische Flussdichte (B)}
\begin{frame}{Magnetische Flussdichte (B)}
  \begin{columns}
    \begin{column}{.5\textwidth}
      \begin{itemize}
        \item Magnetische Flussdichte
        \item magn. Feldlinien pro Fläche
        \item $B = \frac{\mu_0\cdot I}{2\cdot\pi\cdot x}$\\
        $\mu_0=$Magnetische Feldkonstante oder Permeabilitätskonstante im Vakuum. $\mu_0 = 1,2566 \cdot 10^{-6} \frac{N}{A^2}$,
        I =Strom durch den Leiter, x = Abstand zum Leiter
        \item Richtung: willkürlich festgelegt (rechts-Schrauben-Regel)
      \end{itemize}
    \end{column}
    \begin{column}{.5\textwidth}
      \includegraphics[scale=.5]{magFeld_b_3.png}
    \end{column}
  \end{columns}
\end{frame}

% \subsection{Selbstinduktion}
\begin{frame}{Selbstinduktion}
 \begin{itemize}
  \item Vor allem bei Spule
  \item Auswirkung vom magn. Feld auf Stromfluss
  \item Verzögerung bei Anstieg $I(t) = I_0 \cdot \left( 1-e^{-\frac{R}{L}\cdot t}\right)$\\ Abfall $I(t) = I_0 \cdot e^{-\frac{R}{L}\cdot t}$
 \end{itemize}
 Simulation:
...
\begin{frame}{Spule}
  \begin{columns}
    \begin{column}{.4\textwidth}
      \begin{itemize}
        \item Spule = Draht
        \item (viele) Wicklungen
        \item Kern (z. B. Fe ist optional)
      \end{itemize}
    \end{column}
...
% \end{frame}
\section[on/off L]{Ein- und Ausschaltvorgang (Spule)}
\begin{frame}{Einschaltvorgang der Spule}
  \begin{columns}
    \begin{column}{.4\textwidth}
      \begin{itemize}
        \item<1-> Spannung springt von 0\, V auf 5\, V
        \item<2-> Magnetfeld wird sofort aufgebaut
        \item<2-> Strom will fließen
        \item<2-> Magnetfeld wirkt Stromfluss entgegen.
        \item<3-> Spannung an Spule fällt ab
      \end{itemize}
    \end{column}
    \begin{column}{.6\textwidth}
      \only<1>{\includegraphics[scale=.45]{Auf-\_und\_Entladung\_Spule\_1\_1.png}}
      \only<2>{\includegraphics[scale=.45]{Auf-\_und\_Entladung\_Spule\_1\_2.png}}
      \only<3->{\includegraphics[scale=.45]{Auf-\_und\_Entladung\_Spule\_1\_3.png}}
      Ausschnitte aus Bild von Henrik Schumacher via Wikipedia (Spule (Elektrotechnik)) Stand: 03.03.2026
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Formeln (Teil 1)}
  Aufladung der Spule
   \begin{align}
...
\begin{frame}{Formeln (Teil 2)}
$\oint_{\partial A}\vec{E} \cdot d\vec{s} = - \int_A \frac{\partial \vec{B}}{\partial t}\cdot d\vec{A}$
\begin{description}
 \item[links] Spannung (Induziert)
 \item[rechts] Änderung der Flussdichte B (magn. Flussdichte)
\end{description}

\end{frame}


\begin{frame}{Ausschaltvorgang Spule}
  \begin{columns}
    \begin{column}{.4\textwidth}
      \begin{itemize}
        \item Spannung (Versorgung) springt von 5\, V auf 0\, V
        \item Magnetfeld bricht sofort zusammen
        \item Strom will fließen
        \item Magnetfeld bewirkt negative Spannung.
        \item Spannung an Spule fällt ab
      \end{itemize}
    \end{column}
    \begin{column}{.6\textwidth}
      \only<1>{\includegraphics[scale=.45]{Auf-\_und\_Entladung\_Spule\_2\_3.png}}
      \only<2>{\includegraphics[scale=.45]{Auf-\_und\_Entladung\_Spule\_2\_1.png}}
      \only<3->{\includegraphics[scale=.45]{Auf-\_und\_Entladung\_Spule\_2\_2.png}}
      Ausschnitte aus Bild von Henrik Schumacher via Wikipedia (Spule (Elektrotechnik)) Stand: 03.03.2026
    \end{column}
  \end{columns}\end{frame}

\begin{frame}{Aufgaben - Ein- Ausschalten Spule}
\begin{tabular}{|l|l|l|l|l|l|}
  \hline
 $U_0$  & $U_L$    & $\tau$    & t &            R &  L\\ \hline
 12\, V & $U_L(t)$ &           & 10\, s, 22\, s, $3\tau$, 120\, s  & $100 \Omega$ & $220\ mH$\\ \hline
 12\, V & $U_L(t)$ & $391\ \mu $s & $2\tau$, 1,5\, ms, 2\, ms  &  & $470\ mH$\\ \hline
 12\, V & $U_L(t)$ & $254\, \mu $s & $0,5\tau$, $\tau$, $500\, \mu $s & $2,2\ k\Omega$ & \\ \hline
%         & 8,56\, V & & 600 ms & $1,2\ k\Omega$ & $560 \mu F$ \\ \hline
\end{tabular}
\begin{itemize}
  \item berechnen die fehlenden Parameter($\tau$, R, L, $U_0$) und $U_L(t)$
 \item Bei Widerstandswerten und Spulenwerten sind jeweils Werte der E12-Reihe zu bestimmen.
 \item $U_L(t)$ = berechne alle Spannungen für $U_L$ zu den angegebenen Zeitpunkten t.
 \item Als weitere Übungsmöglichkeit: $U_0$ = 15\, V; 18\, V; 20\, V; 24\, V.
\end{itemize}
\end{frame}

% \appendix
\part{Lösungsvorschläge zu Aufgaben}
\section{Lösungsvorschläge zu Elektrisches Feld (Kräfte, Felder)}
\begin{frame}
  \hypertarget{LoesungsVorschlaege}{}
